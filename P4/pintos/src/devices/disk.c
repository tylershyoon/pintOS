...
void
disk_read (struct disk *d, disk_sector_t sec_no, void *buffer, bool is_cache) 
{
  struct channel *c;
  ...
  if (d == filesys_disk && is_cache)
  {
    cache_read (sec_no, buffer);
    return;
  }
...
/* Write sector SEC_NO to disk D from BUFFER, which must contain
   ...
   per-disk locking is unneeded. */
void
disk_write (struct disk *d, disk_sector_t sec_no, const void *buffer, bool is_cache)
{
  struct channel *c;
